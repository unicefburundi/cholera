...
class SearchForm(forms.Form):
    def __init__(self, request=None, *args, **kwargs):
        ...
        if request and request.user.is_authenticated():
            ...
            if level == "BPS":
                PROVINCES = (
                    Province.objects.filter(code=moh_facility)
                    .values_list("id", "name")
                    .distinct()
                )
                DISTRICTS = (
                    District.objects.filter(province__code=moh_facility)
                    .values_list("id", "name")
                    .distinct()
                )
                self.base_fields["province"] = forms.ChoiceField(
                    widget=forms.Select(), choices=PROVINCES
                )
                self.base_fields["districts"] = forms.ChoiceField(
                    widget=forms.Select(),
                    choices=[("", "---------")] + [(str(i), n) for i, n in DISTRICTS],
                    required=False,
                )
                self.base_fields["cds"] = forms.ChoiceField(
            ...
            if level == "BDS":
                ...
                CDSS = (
                    CDS.objects.filter(district=district)
                    .values_list("id", "name")
                    .distinct()
                )
                PROVINCES = (
                    Province.objects.filter(code=district.province.id)
                    .values_list("id", "name")
                    .distinct()
                )
                self.base_fields["province"] = forms.ChoiceField(
                ...
                self.base_fields["cds"] = forms.ChoiceField(
                    widget=forms.Select(),
                    choices=[("", "---------")] + [(str(i), n) for i, n in CDSS],
                    required=False,
                )
            ...
            if level == "CDS":
                ...
                PROVINCES = (
                    Province.objects.filter(code=cds.district.province.id)
                    .values_list("id", "name")
                    .distinct()
                )
                self.base_fields["province"] = forms.ChoiceField(
                ...
                self.base_fields["districts"] = forms.ChoiceField(
                    widget=forms.Select(),
                    choices=[(str(cds.district.id), cds.district.name)],
                    required=False,
                )
                self.base_fields["cds"] = forms.ChoiceField(
                    widget=forms.Select(),
                    choices=[(str(cds.id), cds.name)],
                    required=False,
                )
...
class PatientSearchForm(forms.ModelForm):
    ...
    end_date = forms.DateField(widget=forms.TextInput(attrs={"class": "datePicker"}))
    exit_status = forms.ChoiceField(
        widget=forms.Select(),
        choices=[("", "-----"), ("Pr", "Reference"), ("Pd", "Deces"), ("Pg", "Gueris")],
    )
    ...
    intervention = forms.ChoiceField(
        ...
        choices=[
            ...
            ("DD", "Deces"),
            ("PR", "Reference"),
            ("DESH", "Non Hospitalise"),
        ],
    )
    age = forms.ChoiceField(
        widget=forms.Select(),
        choices=[("", "-----"), ("A1", "Inf 5ans"), ("A2", "Sup 5ans")],
    )
...
class UserCreationMultiForm(MultiModelForm):
    form_classes = OrderedDict(
        (("user", UserCreationForm), ("profile", UserProfileForm2))
    )
